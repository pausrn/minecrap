...
class chunkManager{
  ...
  void createChunkShape(){
    for(int x=0;x<chunkCoords.length;x++) for(int y=0;y<chunkCoords[0].length;y++){
      ...
      chunk cc=chunk[ind];
      cc.createRenderShape(px,py);
      //tot.addChild(cc.renderShape);
    }
  }
  
  void renderShape(){
    for(int i=0;i<chunk.length;i++) shape(chunk[i].renderShape);
  }
  ...
  void updateChunk(){
    ...
    int cy=floor(p.z)>>4;
    if(cx!=px||cy!=py){
      for(int i=0;i<chunk.length;i++) chunk[i].renderShape.translate((cx-px)<<4,0,(cy-py)<<4);
      px=cx;
      py=cy;
    }
  }
